                ...
                <ol class="breadcrumb">
                    ...
                    <li>
                        <a href="${base}${product.path}"
                           title="${product.name}">${abbreviate(product.name, 30, "...")}</a>
                    </li>
                            ...
                            <div class="media-left">
                                <a href="${base}${product.path}" title="${product.name}" target="_blank">
                                    <img class="media-object img-thumbnail"
                                         src="${product.thumbnail!setting.defaultThumbnailProductImage}"
                                         alt="${product.name}">
                                </a>
                            </div>
                        ...
                        <div class="panel-heading">
                            <ul class="type clearfix">
                                [@review_count productId = product.id]
                                    <li[#if type == null] class="current"[/#if]>${message("shop.review.allType")}
                                        (${count})
                                    </li>
                                [/@review_count]
                                ...
                                [#list types as type]
                                    [@review_count productId = product.id type = type]
                                        <li[#if type == currentType] class="current"[/#if]
                                                val="${type}">${message("Review.Type." + type)}(${count})
                                        </li>
                                    [/@review_count]
                                [/#list]
                            </ul>
                                    ...
                                    <div class="media-body">
                                        ...
                                        [#if review.replyReviews?has_content]
                                            [#list review.replyReviews as replyReview]
                                                <p class="review-reply">${replyReview.store.name}
                                                    : ${replyReview.content}</p>
                                            [/#list]
                                        [/#if]
                                        ...
                                        <span class="text-gray-dark"
                                              title="${review.createdDate?string("yyyy-MM-dd HH:mm:ss")}">${review.createdDate?string("yyyy-MM-dd")}</span>
                                    </div>
                                </div>
                            [/#list]
                        </div>
