...
<head>
	...
	<title>${message("business.product.edit")} - Powered By </title>
						...
						<tr data-ids="<%-ids.join(",")%>" <%if (initSkuValue != null) {%>title="${message("Sku.sn")}: <%-initSkuValue.sn%>"<%}%>>
							...
							<td>
								<div class="checkbox">
									...
									<input name="skuList[<%-i%>].isDefault" class="is-default" type="checkbox" <%-!sample ? " disabled" : ""%> value="true"<%-isDefault ? " checked" : ""%><%-!isEnabled ? " disabled" : ""%>>
									<label></label>
								</div>
							</td>
							<td>
				...
				function buildSkuTable(skuValues) {
					...
					if (skuValues == null) {
						...
						$sku.find("tr").each(function() {
							var $element = $(this);
							var  $cost = $element.find("input.cost");
