                ...
                <dl class="clearfix">
                    <dt>
                        <a href="${base}${storeProductCategory.path}"
                           title="${storeProductCategory.name}">${abbreviate(storeProductCategory.name, 15, "...")}</a>
                    </dt>
                    [@store_product_category_children_list storeProductCategoryId = storeProductCategory.id storeId = store.id recursive = false]
                        [#list storeProductCategories as storeProductCategory]
                            <dd>
                                <a class="text-overflow" href="${base}${storeProductCategory.path}"
                                   title="${storeProductCategory.name}">${storeProductCategory.name}</a>
                            </dd>
                        [/#list]
