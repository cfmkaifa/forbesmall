        ...
        [#escape x as x?js_string]
            <script>
                $().ready(function () {
                    ...
                    // 修改
                    $document.on("success.mall.modifyCart", function (event, data) {
                    ...
                    // 移除
                    $document.on("success.mall.removeCart", function (event, data) {
                        var $element = $(event.target);
                    ...
                    // 清空
                    $document.on("success.mall.clearCart", function () {
                        location.reload(true);
                    });
    ...
    <div class="container">
        [#if currentCart?? && !currentCart.isEmpty()]
                        ...
                        <tr>
                            <td>
                                <div class="media">
                                    <div class="media-left media-middle">
                                        <a class="product-image" href="${base}${cartItem.sku.path}"
                                           title="${cartItem.sku.name}">
                                            <img class="media-object img-thumbnail"
                                                 src="${cartItem.sku.thumbnail!setting.defaultThumbnailProductImage}"
                                    ...
                                    <div class="media-body media-middle">
                                        <h5 class="media-heading">
                                            <a href="${base}${cartItem.sku.path}"
                                               title="${cartItem.sku.name}">${cartItem.sku.name}</a>
                                        </h5>
                            ...
                            <td>
                                <div class="spinner input-group input-group-sm" data-trigger="spinner">
                                    <input class="form-control" type="text" value="${cartItem.quantity}" maxlength="5"
                                           data-rule="quantity" data-min="1" data-max="10000" data-action="modifyCart"
                                           data-sku-id="${cartItem.sku.id}">
            ...
            <div class="bar">
                <div class="row">
                    <div class="col-xs-1">
                        <a id="clear" class="clear" href="javascript:;"
                           data-action="clearCart">${message("shop.cart.clear")}</a>
                    </div>
                    ...
                    <div class="col-xs-1">
                        <button class="btn btn-primary btn-lg btn-block" type="button"
                                data-action="checkout">${message("shop.cart.checkout")}</button>
                    </div>
                </div>
            </div>
        [#else]
