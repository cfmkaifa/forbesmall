...
<head>
    ...
    <script id="receiverListItemTemplate" type="text/template">
        ...
        <p class="text-overflow" title="<%-receiver.areaName%><%-receiver.address%>">
            <%-receiver.areaName%><%-receiver.address%></p>
        ...
        [#escape x as x?js_string]
            <script>
                $().ready(function () {
                    ...
                    // 添加收货地址成功
                    $addReceiverForm.on("success.mall.ajaxSubmit", function (event, data) {
                    ...
                    $balance.change(function () {
                        ...
                        if (/^\d+(\.\d{0,${setting.priceScale}})?$/.test($element.val())) {
                            ...
                            amount ? amount : ${currentUser.availableBalance};
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label class="col-xs-3 control-label item-required">${message("Receiver.consignee")}
                                    :</label>
                                <div class="col-xs-8">
                            ...
                            <div class="form-group">
                                <label class="col-xs-3 control-label item-required">${message("Receiver.address")}
                                    :</label>
                                <div class="col-xs-8">
                            ...
                            <div class="form-group">
                                <label class="col-xs-3 control-label item-required">${message("Receiver.zipCode")}
                                    :</label>
                                <div class="col-xs-8">
                            ...
                            <div class="form-group">
                                <label class="col-xs-3 control-label item-required">${message("Receiver.phone")}
                                    :</label>
                                <div class="col-xs-8">
        ...
        <form id="orderForm" class="ajax-form form-horizontal" action="${base}/order/create" method="post">
            ...
            [#if isDelivery]
                <input id="receiverId" name="receiverId"
                       type="hidden"[#if defaultReceiver??] value="${defaultReceiver.id}"[/#if]>
            [/#if]
            <input id="paymentMethodId" name="paymentMethodId"
                   type="hidden"[#if defaultPaymentMethod??] value="${defaultPaymentMethod.id}"[/#if]>
            <input id="shippingMethodId" name="shippingMethodId"
                   type="hidden"[#if defaultShippingMethod??] value="${defaultShippingMethod.id}"[/#if]>
            [#if isDelivery]
                    ...
                    <div class="panel-body">
                        ...
                        <ul class="receiver-list">
                            [#list currentUser.receivers as receiver]
                                <li[#if receiver == defaultReceiver] class="active"[/#if]
                                        data-receiver-id="${receiver.id}">
                                    <h5 class="text-overflow" title="${receiver.consignee}">${receiver.consignee}</h5>
                                    <p class="text-gray-dark">${receiver.phone}</p>
                                    <p class="text-overflow"
                                       title="${receiver.areaName}${receiver.address}">${receiver.areaName}${receiver.address}</p>
                                    <p class="text-gray-dark">${receiver.zipCode}</p>
                                </li>
                            [/#list]
                        </ul>
            ...
            <div id="paymentMethod"
                 class="payment-method[#if amountPayable <= 0] hidden-element[/#if] panel panel-default">
                <div class="panel-heading">
                ...
                <div class="panel-body">
                    <ul class="payment-method-list">
                        [#list paymentMethods as paymentMethod]
                            <li[#if paymentMethod == defaultPaymentMethod] class="active"[/#if]
                ...
                <div id="shippingMethod" class="shipping-method panel panel-default">
                    ...
                    <div class="panel-body">
                        <ul class="shipping-method-list">
                            [#list shippingMethods as shippingMethod]
                                <li[#if shippingMethod == defaultShippingMethod] class="active"[/#if]
            ...
            [#--<div class="panel panel-default">
                <div class="panel-body">
                            ...
                            --][#--<div id="invoiceWrapper">
                                <div class="form-group">
                                    ...
                                    [#if setting.isTaxPriceEnabled]
                                        <div class="col-xs-10">
                                            <p class="text-gray form-control-static">${message("Order.tax")}
                    ...
                    [#if currentUser.availableBalance > 0]
                        <div id="balanceWrapper"[#if amount <= 0] class="hidden-element"[/#if]>
                            <div class="form-group">
                                ...
                                <div class="col-xs-10">
                                    <p class="text-gray form-control-static">${message("shop.order.balance")}
                                        : ${currency(currentUser.availableBalance, true, true)}</p>
                                </div>
                            </div>
                            <div class="form-group hidden-element">
                                <label class="col-xs-1 control-label">${message("shop.order.useAmount")}</label>
                                <div class="col-xs-4">
                                    <input id="balance" name="balance" class="form-control" type="text" value="0"
                                           maxlength="16" onpaste="return false;" disabled>
                                </div>
                            </div>
                        </div>
                    [/#if]
                    <div class="form-group">
                        ...
                        <div class="col-xs-4">
                            <input name="memo" class="form-control" type="text" maxlength="200"
                                   placeholder="${message("shop.order.memoPlaceholder")}">
                        </div>
                    </div>
                </div>
            </div>--]
                                    ...
                                    <div class="media-left media-middle">
                                        <a href="${base}${orderItem.sku.path}" title="${orderItem.sku.name}">
                                            <img class="media-object img-thumbnail"
                                                 src="${orderItem.sku.thumbnail!setting.defaultThumbnailProductImage}"
                                    ...
                                    <div class="media-body media-middle">
                                        <h5 class="media-heading">
                                            <a href="${base}${orderItem.sku.path}"
                                               title="${orderItem.sku.name}">${orderItem.sku.name}</a>
                                        </h5>
                ...
                <div class="row">
                    ...
                    <div class="col-xs-1">
                        <button class="btn btn-primary btn-lg btn-block"
                                type="submit">${message("shop.order.checkout")}</button>
                    </div>
                </div>
            </div>
        </form>
